...
function $GrxNoticesPage(){
	...
	ha.push('<div class="pages">')
	ha.push('<div class="pages_l">共'+_totalRows+'条记录,');
	ha.push('每页显示<select name="pageSize"  onchange="goNoticesPage(1)"><option value="5">5</option><option value="10">10</option><option value="20">20</option><option value="50">50</option></select>条</div>');
	ha.push('<div class="pages_r">');
	ha.push('<ul>');	
    ha.push('<li><a onclick="goNoticesPage('+(_pageNumber -1)+')">上一页</a></li>');
    ha.push('<li><a onclick="goNoticesPage(1)" class="now">首页</a></li>');
    ...
    ha.push('<li><a onclick="goNoticesPage('+(_pageNumber+1)+')" class="now">下一页</a></li>');
	ha.push('</ul>');
	ha.push('</div>');
	ha.push('</div>');
...
function goNoticesPage(pageNumber){
	...
	$.ajax({
		url      : "/aboutUsController/loadnoticesListByAsync?currentPage=" + pageNumber + "&pageSize=" + pageSize,
		dataType : "html",
		success  : function(data){
			$("#noticesPage").html(data);
		}
	})
}
</script>
