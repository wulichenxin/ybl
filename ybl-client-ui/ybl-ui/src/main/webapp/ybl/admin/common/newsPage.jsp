...
function $GrxPage(){
	...
	ha.push('每页显示<select name="pageSize"  onchange="goNewsPage(1)"><option value="5">5</option><option value="10">10</option><option value="20">20</option><option value="50">50</option></select>条</div>');
	ha.push('<div class="pages_r">');
	ha.push('<ul>');	
    ha.push('<li><a onclick="goNewsPage('+(_pageNumber -1)+')">上一页</a></li>');
    ha.push('<li><a onclick="goNewsPage(1)" class="now">首页</a></li>');
    ...
    ha.push('<li><a onclick="goNewsPage('+(_pageNumber+1)+')" class="now">下一页</a></li>');
	ha.push('</ul>');
	ha.push('</div>');
	ha.push('</div>');
...
function goNewsPage(pageNumber){
	...
	$.ajax({
		url      : "/aboutUsController/loadnewsListByAsync?currentPage=" + pageNumber + "&pageSize=" + pageSize,
		dataType : "html",
