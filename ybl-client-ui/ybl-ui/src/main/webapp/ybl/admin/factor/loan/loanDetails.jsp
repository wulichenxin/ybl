...
<script type="text/javascript">
	$(function() {
		...
		var auditype = "${auditType}";
		var ids = "${ids}";
		
		$("#ybl_risk_auditDetail_audit").click(function(){
			audit(auditype,ids);
		});
...
<body>
	<!--top start -->
	<jsp:include page="../../common/top.jsp?step=${step }" />
	<!--top end -->
	...
	<div class="vip_conbody body_cbox">
		<div class="tabnav">
				...
				<div class="rzzt">
					<ul>
						...
						<li class=" w472">
							<div class="input_box">
								<span><spring:message code="sys.client.repaymentType"/><!-- 还款方式 -->：</span>
									...
									<input  type="text" class="w200 text_gary"  
										value='<spring:message code="sys.client.oneTime.payment"/>' readonly="readonly"/>
							</div>
						</li>
						<li class="clear w472">
						     <div class="input_box">
						   ...
						   <div class="input_box">
								...
								<div>
									<spring:message code="sys.client.yearRate"/><!-- 年利率 --><input placeholder="10" type="text"
										...
										class="text w50 mr10 ml10" value='<fmt:formatNumber value="${financeApply.rate_}" pattern="0.00"  />' readonly="readonly"/>%
								</div>
							</div>
						</li>
						<li class="clear w472"><div class="input_box">
								...
								<div>
									<spring:message code="sys.client.dailyInterest"/><!-- 日利率 --><input placeholder="10" type="text"  
										class="text w50 mr10 ml10" value='<fmt:formatNumber value="${financeApply.overdue_rate}" pattern="0.00"  />' readonly="readonly"/>%
								</div>
							</div></li>
						<li class="w472"><div class="input_box">
								...
								<div>
									<spring:message code="sys.client.yearRate"/><!-- 年利率 --><input placeholder="10" type="text"
										class="text w50 mr10 ml10" value='<fmt:formatNumber value="${financeApply.manage_rate}" pattern="0.00"  />' readonly="readonly"/>%
								</div>
							</div></li>
						<li class="w472"><div class="input_box">
								...
								<div>
									<spring:message code="sys.client.dailyInterest"/><!-- 日利率 --><input placeholder="10" type="text"
										class="text w50 mr10 ml10" value='<fmt:formatNumber value="${financeApply.penalty_rate}" pattern="0.00"  />' readonly="readonly"/>%
								</div>
							</div></li>
						<li class="w472"><div class="input_box"> 
								<span><spring:message code="sys.client.financingProportion"/><!-- 融资比例 -->：</span><input placeholder="20" type="text"
									class="text w50 mr10" value='<fmt:formatNumber value="${financeApply.finance_ratio}" pattern="0.00"  />' readonly="readonly"/>%
							</div></li>
					</ul>
				</div>
			</div>
					...
					<ul>
						...
						<li class="w472"><div class="input_box">
								<span><spring:message code="sys.client.financeApplyMoney"/><!-- 申请贷款金额 -->：</span><input placeholder="20" type="text"
									class="text w100 mr10" value='<fmt:formatNumber value="${financeApply.apply_amount / 10000 }" pattern="###,##0.00"  />' readonly="readonly"/><spring:message code="sys.client.tenThousand"/><!-- 万元 -->
							</div></li>
						<li class="w472"><div class="input_box">
								<span><spring:message code="sys.client.maxLoanMoney"/><!-- 最大可贷金额 -->：</span><input placeholder="20" type="text"
									class="text w100 mr10" value='<fmt:formatNumber value="${maxLoanMoney / 10000}" pattern="###,##0.00"  />' readonly="readonly"/><spring:message code="sys.client.tenThousand"/><!-- 万元 -->
							</div></li>
						<li class="w472">
							<div class="input_box">
						...
						<li><div class="input_box">
								<span><spring:message code="sys.client.repaymentTime"/><!-- 还款日期 -->：</span>
									...
									class="w300 text_gary" value="<fmt:formatDate value="${dateValue}" pattern="yyyy-MM-dd HH:mm:ss" />" readonly="readonly"/>
				...
				<div class="table_con table_border ">
					<table width="100%" border="0" cellspacing="0" cellpadding="0"
						...
						<tr>
							...
							<th><spring:message code="sys.client.voucherUnit"/><!-- 凭证面额（元） --></th>
						...
						<tbody>
							<c:forEach items="${voucherList}" var="bean" varStatus="state">     
			                        <tr>
						                ...
						                <td><fmt:formatNumber value="${bean.amount / 10000}" pattern="###,##0.00"  /></td>
			...
			<c:if test="${not empty disbursementLineList  }">
			<div class="rzsq_box">
				<h1>支付信息</h1>  
				...
				<div class="table_con table_border ">
					<table width="100%" border="0" cellspacing="0" cellpadding="0"
						...
						<tbody>
							<c:forEach items="${disbursementLineList}" var="bean" varStatus="state">
	                        <tr>
	                            <td>${state.count}</td>
				                <td><fmt:formatNumber value="${bean.apply_amount / 10000}" pattern="#,##0.00" maxFractionDigits="2"/></td>
				                <td><fmt:formatNumber value="${bean.finance_ratio }" pattern="#,##0.00" maxFractionDigits="2"/>%</td>
				                <%-- <td><fmt:formatNumber value="${bean.balance_amount /10000}" pattern="#,##0.00" maxFractionDigits="2"/></td> --%>
				                <td><fmt:formatNumber value="${bean.loan_fee }" pattern="#,##0.00" maxFractionDigits="2"/></td>
				                <td><fmt:formatNumber value="${bean.manage_fee }" pattern="#,##0.00" maxFractionDigits="2"/></td>
				                <td>
				                	<jsp:useBean id="dateValue4" class="java.util.Date" />
									<jsp:setProperty name="dateValue4" property="time" value="${bean.pay_time }" />
									<fmt:formatDate value="${dateValue4}" pattern="yyyy-MM-dd" />
				                </td>
				                <td><fmt:formatNumber value="${bean.amount_ /10000}" pattern="#,##0.00" maxFractionDigits="2"/></td>
				                <td>${bean.comment_}</td>
				                <td>
								   <div class="qy">
										<a href="javascript:view('${bean.url_}');"><spring:message code="sys.client.preview"/><!-- 预览 --></a>
			...
			<div class="rzsq_box">
				...
				<div class="table_con table_border ">
					<table width="100%" border="0" cellspacing="0" cellpadding="0"
						...
						<tbody>
							<c:forEach items="${repaymentList}" var="bean" varStatus="state">
	                        <tr>
	                            <td>${state.count}</td>
	                            <td>
	                            	<c:if test="${bean.toRepayment < 0 || bean.toRepayment == '0.0000'}">
	                            	0.00
	                            	</c:if>
	                            	<c:if test="${bean.toRepayment > 0 }">
				                		<fmt:formatNumber value="${bean.toRepayment / 10000}" pattern="#,##0.00" maxFractionDigits="2"/>
				                	</c:if>
				                </td>
				                <td><fmt:formatNumber value="${bean.actual_amount / 10000}" pattern="#,##0.00" maxFractionDigits="2"/></td>
				                <td><fmt:formatNumber value="${bean.drawback_amount / 10000}" pattern="#,##0.00" maxFractionDigits="2"/></td>
				                <td><fmt:formatNumber value="${bean.repayment_amount / 10000}" pattern="#,##0.00" maxFractionDigits="2"/></td>
				                <td>
				                	<jsp:useBean id="dateValue5" class="java.util.Date" />
									<jsp:setProperty name="dateValue5" property="time" value="${bean.pay_time }" />
									<fmt:formatDate value="${dateValue5}" pattern="yyyy-MM-dd" />
								</td>
				                <td>${bean.comment_ }</td>
				                <td>
								   <div class="qy">
			...
			<div class="rzsq_box" style="display:none;">
				<h1><spring:message code="sys.client.contract"/></h1>  <!-- 合同信息 -->
				<div class="rzzt">
					<ul>
						<li><div class="input_box">
								<span><spring:message code="sys.client.selectFile"/>：</span><input type="file" class=" f16"
			...
			<c:if test="${not empty auditType && auditType < 4 }">
				...
				<div class="but_left">
						...
						<%-- <c:if test="${ auditType==-1}">
							<a  disabled="disabled" class="bgGray" style="background-color: #ccc;"><spring:message code="sys.client.alreadyPaid"/><!-- 已支付 --></a>
						</c:if> --%>
						
						<c:if test="${ auditType==-2}">
							<a  disabled="disabled" class="bgGray" style="background-color: #ccc;"><spring:message code="sys.client.auditOperator"/><!-- 不通过 --></a>
						</c:if>
						
						<c:if test="${ auditType!=-1 && auditType!=-2}">
							<sun:button id="ybl_risk_auditDetail_audit" tag='a' clazz="but_ico3" i18nValue="sys.client.auditOperator" />
						</c:if>
						
						
						<a  onclick="javascript:history.go(-1);" >返回列表</a>
