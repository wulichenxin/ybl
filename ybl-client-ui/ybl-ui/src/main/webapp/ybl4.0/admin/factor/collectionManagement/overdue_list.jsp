...
<html>
	...
	<body>
		<div class="Bread-nav">
			<div class="w1200"><img class="mr10" src="${app.staticResourceUrl}/ybl4.0/resources/images/rzf_bre_icon.png" />收款管理--逾期项目</div>
		</div>
		<form action="/factorCollectionManagementController/overdue/list.htm" id="pageForm" method="post">
						...
						<table>
							...
							<c:forEach items="${page.records}" var="obj" varStatus="index">
				              	<tr>
									...
									<td class="maxwidth">${obj.overdueDays}</td>
									<td class="maxwidth">${obj.overdueFee}</td>
									<td class="maxwidth"><c:if test="${obj.repaymentStatus eq 3}">已逾期</c:if><c:if test="${obj.repaymentStatus eq 1}">待还款</c:if></td>
									...
									<td class="maxwidth">
										<c:if test="${obj.confirmStatus eq 1 && not empty obj.actualRepaymentDate}">
											<a href="/factorCollectionManagementController/${obj.repaymentId}/2/preConfirmeRepayment.htm" class="btn-modify mr10">确认</a>
										</c:if>
										<a href="/factorCollectionManagementController/developing.htm" class="btn-modify mr10" >催收</a>
										<c:if test="${empty obj.actualRepaymentDate}">
											<a href="javascript:;" class="btn-modify mr10" onclick="badDebt(${obj.repaymentId})">转坏账</a>
										</c:if>
									</td>
								</tr>
			              </c:forEach>
				...
				success:function(data){
					if(data.responseTypeCode == "success"){
						alert(data.info,function(){
							location.href='/factorCollectionManagementController/overdue/list.htm';
						});
					}else{
						alert(data.info);
					}					
		...
		$(".subView").click(function(){
			...
			param={"id":lid,"url":"/factorCollectionManagementController/overdue/list.htm"};
