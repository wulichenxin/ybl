...
<html>
	...
	<body>
		<div class="Bread-nav">
			<div class="w1200"><img class="mr10" src="${app.staticResourceUrl}/ybl4.0/resources/images/rzf_bre_icon.png" />风控初审</div>
		</div>
		<form action="/factorRiskManagementController/firstAudit/list.htm" id="pageForm" method="post">
					...
					<div class="scrollbox">
						<table>
							...
							<c:forEach items="${page.records}" var="obj" varStatus="index">
				              	<tr>
									<td class="toggletr maxwidth">${index.count}</td>
									<td class="maxwidth"><a style="cursor: pointer;" target="_blank" onclick="javascript:window.location.href=('<%=basePath%>factorRiskManagementController/${obj.financingApplyId }/${obj.subFinancingApplyId }/1/financingApplyDetail.htm');" class="order-a">${obj.financingOrderNumber}</a></td>
									...
									<td class="maxwidth">
										<c:if test="${obj.financingStatus eq 3}">待资方初审</c:if>
									</td>
									<td class="maxwidth"><a onclick="javascript:window.location.href=('<%=basePath%>factorRiskManagementController/${obj.financingApplyId }/${obj.subFinancingApplyId }/1/preCreateAuditHistory.htm');" class="btn-modify mr10">审核</a></td>
								</tr>
			              </c:forEach>
						</table>
					</div>
