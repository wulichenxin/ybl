...
<html>
	...
	<body>
		...
		<p class="per_title"><span>已回款明细</span></p>
		<div class="mt40">
			<form action="/factorRepaymentPlanController/finish.htm" id="pageForm" method="post">
				<div class="ybl-info">
							...
							<table>
								...
								<c:forEach items="${list}" var="obj" varStatus="index">
					              	<tr>
										...
										<td title="${obj.loanApplyOrderNo}" class="maxwidth"><a href="javascript:;" class="order-a details" uid="${obj.loanId }" orderNo="${obj.loanApplyOrderNo }" batchId="${obj.paymentBatchId }" subId="${obj.subFinancingApplyId }">${obj.loanApplyOrderNo}</a></td>
										...
										<td>
											<c:if test="${obj.confirmStatus eq 2}">已还款确认</c:if>
										</td>
										...
										<c:if test="${obj.confirmStatus eq 2}">
											<td>已确认</td>
										</c:if>
										<c:if test="${obj.confirmStatus eq 1}">
											<td>
												<a class="details" href="#" class="btn-modify mr10 tip" uid="${obj.loanId }" orderNo="${obj.loanApplyOrderNo }" batchId="${obj.paymentBatchId }" subId="${obj.subFinancingApplyId }" onclick="">查看详情</a>
											</td>
										</c:if>
	 								</tr>
				              </c:forEach>
