...
//验证时间日期yyyy-MM-dd hh:mm:ss
$.validator.addMethod("datetimeCN",function(value,element){
	return this.optional(element) ||  /^(?:19|20)[0-9][0-9]-(?:(?:0[1-9])|(?:1[0-2]))-(?:(?:[0-2][1-9])|(?:[1-3][0-1])) (?:(?:[0-2][0-3])|(?:[0-1][0-9])):[0-5][0-9]:[0-5][0-9]$/.test(value);
},"请输入正确的日期时间格式 例如2016-08-06 12:12:12");

$.validator.addMethod("longitude",function(value,element){
//	var patternLongitude = /^[+-]?((\d|[1-9]\d|1[0-7]\d)[\s\-,;°度](\d|[0-5]\d)[\s\-,;′分](\d|[0-5]\d)(\.\d{1,2})?[\s\-,;\″秒]?$)|(180[\s\-,;°度]0[\s\-,;′分]0[\s\-,;\″秒]?$)/;
	var patternLongitude = /^[+-]?((\d|[1-9]\d|1[0-7]\d)°(\d|[0-5]\d)'(\d|[0-5]\d)(\.\d{1,2})?"?$)|(180°0'0"?$)/;
	return this.optional(element) ||  patternLongitude.test(value);
},"请输入正确的经度格式 取值范围-180°0'0\" 至 180°0'0\"");

$.validator.addMethod("latitude",function(value,element){
//	var patternLatitude = /^[+-]?((\d|[1-8]\d)[\s\-,;°度](\d|[0-5]\d)[\s\-,;'分](\d|[0-5]\d)(\.\d{1,2})?[\s\-,;\"秒]?$)|(90[\s\-,;°度]0[\s\-,;'分]0[\s\-,;\"秒]?$)/;
	var patternLatitude = /^[+-]?((\d|[1-8]\d)°(\d|[0-5]\d)'(\d|[0-5]\d)(\.\d{1,2})?"?$)|(90°0'0"?$)/;
	return this.optional(element) ||  patternLatitude.test(value);
},"请输入正确的纬度格式 取值范围-90°0'0\" 至 90°0'0\"");
...
//-------------------------------------------------------自定义-----------------------------------------------------------------------
	...
	//密码值长度校验
	jQuery.validator.addMethod("passWordLengthCheck", function(value, element,params) {//自定义参数
		if(value.length == 0) {
			return true;
		} else {
